...
from inference_config import TYPE_GAP, TYPE_R_BASE, TYPE_NAMES_EN as TYPE_NAMES
...
def build_reverse_gb_map(encoding_map_path):
    """从 encoding_map.json 构建 GB 值 -> face_id 的反向映射。

    encoding_map.json 中 "faces" 字段格式:
      {"1": {"G": 0, "B": 0}, "2": {"G": 0, "B": 32}, ...}
    表示 face_id=1 编码为 GB=(0,0), face_id=2 编码为 GB=(0,32), ...

    Returns:
        dict: {raw_gb_value: face_id, ...}
    """
    with open(encoding_map_path, "r") as f:
        data = json.load(f)
    faces = data.get("encoder", {}).get("faces", {})
    reverse = {}
    for fid_str, gb in faces.items():
        raw_gb = (int(gb["G"]) << 8) | int(gb["B"])
        reverse[raw_gb] = int(fid_str)
    return reverse
